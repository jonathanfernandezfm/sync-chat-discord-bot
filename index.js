...
async function sendMessageWebHook(guild_id, channel_id, message){
	...
	try {
		...
		await webhook.send({
			...
			db.run(joinGroup, [message.id,webhookMessage.id, webhookMessage.webhookId, webhookMessage.guildId, webhookMessage.channelId])
		});
	} catch (error) {
...
async function checkWebHook(serverId, channelId){
	const guild = await client.guilds.cache.get(serverId);
                            ...
                            if (webhooks.size === 0) {
								channelweb.createWebhook({name: "uwu",
